-- ════════════════════════════════════════════════════════════════════════════
-- Feed de Inicio: excluir promos de cumpleaños del RPC por distancia.
-- Antes se devolvían y el cliente las quitaba, lo que reducía el conteo de la
-- página y cortaba la paginación (hasMore basado en length == page_size).
-- Las promos de cumpleaños tienen su propio feed (get_birthday_promotions).
-- Idéntico a 20260608_promotions_time_band salvo: AND p.type <> 'birthday'.
-- ════════════════════════════════════════════════════════════════════════════
...
DROP FUNCTION IF EXISTS public.get_promotions_by_distance(
  ...
  boolean, boolean, text, text[], integer, text, text, boolean, uuid, text);
...
AS $function$
  ...
  WHERE
    ...
    -- Cumpleaños tiene su propio feed; no debe contar en el feed normal.
    AND p.type <> 'birthday'
