-- Fix: en user_cats, "WHERE id = v_user_id" era ambiguo (id también es columna
-- de salida del RETURNS TABLE) → get_ads_for_user fallaba y no salían anuncios.
-- Se califica con alias (p.id). Mismas columnas de retorno → CREATE OR REPLACE.
