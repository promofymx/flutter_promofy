-- ════════════════════════════════════════════════════════════════════════════
-- Fix: el feed de "Lugares" no devolvía `schedule` ni `payment_methods`, por lo
-- que los filtros cliente "Abiertos ahora" y "Método de pago" nunca excluían
-- nada (e.schedule/e.payment_methods llegaban null al modelo).
--
-- Se recrea get_establishments_by_distance agregando esas 2 columnas al retorno.
-- El cuerpo es idéntico al de 20260608_establishments_time_band.sql salvo las
-- 2 columnas extra en RETURNS TABLE y en el SELECT.
-- ════════════════════════════════════════════════════════════════════════════

DROP FUNCTION IF EXISTS public.get_establishments_by_distance(
  double precision, double precision, double precision, text, boolean, integer, integer[], uuid, integer, integer, text);
...
RETURNS TABLE(
  id uuid, name text, logo_url text, address text,
  lat double precision, lng double precision, distance_meters double precision,
  is_favorited boolean, active_promos_count bigint, has_flash_promos boolean,
  schedule jsonb, payment_methods jsonb
)
LANGUAGE sql
...
AS $function$
  SELECT
    ...
    e.schedule::jsonb            AS schedule,
    to_jsonb(e.payment_methods)  AS payment_methods
