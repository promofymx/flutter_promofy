-- ════════════════════════════════════════════════════════════════════════════
-- get_ads_for_user v2: devolver datos de la PROMO (nombre/foto) cuando la
-- campaña publicita una promoción, y el LOGO del establecimiento (para
-- anuncios de establecimiento). Así el front muestra:
--   • Anuncio de promoción  → imagen + datos de la promo (como card normal)
--   • Anuncio de establec.  → logo + nombre del negocio
-- ════════════════════════════════════════════════════════════════════════════

-- Cambian las columnas del retorno → hay que DROP antes de recrear.
DROP FUNCTION IF EXISTS public.get_ads_for_user(float8, float8, text, int);
...
GRANT EXECUTE ON FUNCTION public.get_ads_for_user(float8, float8, text, int) TO authenticated, anon;
